...
public class App {
    ...
    // Porta HTTP onde o servidor escuta
    static final int PORT = 8080;
    // Nome do arquivo CSV que persiste as tarefas
    static final String CSV = "data_tasks.csv";
    // Capacidade máxima de tarefas (array estático)
    static final int MAX = 5000;

    // Estruturas internas para armazenar tarefas em memória
    ...
    static int n = 0; // número atual de tarefas

    public static void main(String[] args) throws Exception {
        // Carrega tarefas do CSV (se existir)
        carregar();

        // Cria e configura o servidor HTTP embutido
        HttpServer server = HttpServer.create(new InetSocketAddress(PORT), 0);
        server.createContext("/", new RootHandler());
        server.createContext("/api/tasks", new ApiTasksHandler());
        server.setExecutor(null); // executor padrão (single thread pool)
        System.out.println("Servindo em http://localhost:" + PORT);
        server.start();
    }

    /**
     * Handler para a raiz "/" que serve a página HTML estática embutida (INDEX_HTML).
     * Apenas aceita GET e retorna 405 para outros métodos.
     */
     ...
     * aplicável. Exceções entram no catch geral que retorna 500.
     */
        ...
        @Override public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                ...
                if ("POST".equals(method) && "/api/tasks".equals(path)) {
                    // Lê o corpo da requisição e extrai titulo/descricao (JSON simples)
                ...
                if ("DELETE".equals(method) && path.startsWith("/api/tasks/")) {
                    // Remove item movendo os elementos seguintes para preencher o espaço
...
// Função de escape para evitar XSS simples no frontend
...
""";
    ...
    /**
     * Carrega o CSV para as estruturas internas.
     * CSV esperado: id;titulo;descricao;status;criadoEm
     */
    static void carregar() {
        n = 0;
        Path p = Paths.get(CSV);
        if (!Files.exists(p)) return; // nada a fazer se não existir
        try (BufferedReader br = Files.newBufferedReader(p, StandardCharsets.UTF_8)) {
            String line;
            while ((line = br.readLine()) != null) {
                if (line.isBlank() || line.startsWith("id;")) continue; // pula cabeçalho
                String[] a = splitCsv(line);
                if (a.length < 5) continue; // linha inválida
                if (n >= MAX) break; // atingiu capacidade
                ids[n] = a[0];
                titulos[n] = a[1];
                descrs[n] = a[2];
    ...
    /**
     * Persiste o estado atual para o CSV. Cria diretório se necessário.
     */
    static void salvar() {
        Path p = Paths.get(CSV);
        try {
    ...
    /**
     * Cria nova tarefa em memória e retorna o mapa representando a tarefa.
     */
    ...
    // Busca índice no array pelo id
    static int findIdxById(String id){
        for (int i=0;i<n;i++) if (ids[i].equals(id)) return i;
        return -1;
    }

    // Monta um Map com os campos da tarefa no índice i
    ...
    // Converte todas as tarefas para um JSON array manualmente
    ...
    // Serializa um Map de tarefa para JSON (manual)
    ...
    /**
     * Pega um campo simples de um JSON do tipo objeto plano {"a":1, "b":"x"}
     * Implementação caseira: não suporta todos os casos (ex: valores complexos)
     */
    ...
    // Remove aspas externas e unescape simples
    ...
    // Escape para JSON (apenas alguns caracteres básicos)
